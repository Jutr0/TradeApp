...
const ShopPacksList = () => {
    ...
    const [openModal,setOpenModal] = useState<IImgModal>({open:false,image:""})
  ...
  return (
      <>
      <Modal open={openModal.open} onClose={()=>setOpenModal({open:false, image:""})} style={{display:"flex",justifyContent:"center", alignItems:"center"}} >
          <img src={openModal.image}></img>
      </Modal>
    <List>

    {packsData?(packsData.map(step=><ShopPackListItem {...step} setOpenModal={(open:boolean,image:string)=>setOpenModal({open,image})} />)):null}
